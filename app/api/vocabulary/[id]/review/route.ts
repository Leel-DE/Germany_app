...
export async function POST(req: NextRequest, ctx: RouteContext<"/api/vocabulary/[id]/review">) {
  ...
  if (!progress) {
    return NextResponse.json({ error: "Card is not assigned to this user" }, { status: 404 });
  }
  ...
  const now = new Date();
  const nextDueDate = result.nextDueDate.toISOString().slice(0, 10);
